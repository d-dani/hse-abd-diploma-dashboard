...
if _is_demo():
    ...
    st.info(
        ...
        "для полного набора нужно запустить дашборд локально с реальным хранилищем",
        icon="ℹ️",
    )
st.warning(
    "Дашборд оптимизирован для светлой темы, "
    "если часть надписей не отображается, переключитесь ⋮ → Settings → Light"
)
...
with tab1:
    ...
    with left:
        ...
        а также межд­оменные соотношения и соцдем-характеристики.
        """)
        ...
        <div class="info-box">
            ...
            Общий объём источника: 2,8 ТБ. В рабочем контуре использовались данные за 3 месяца (~300 ГБ):
            950 тыс. отзывов · 309 тыс. авторов · 733 млн marketplace-событий.<br><br>
            ID пользователя (user_id) согласован между доменами, что позволяет строить единый профиль пользователя.
        </div>
        """, unsafe_allow_html=True)
